...
trait FlowGraphs {
  ...
  def parseCsv(maximumLineLength: Int = 256): Flow[ByteString, Row, Unit] =
    Framing.delimiter(ByteString("\n"), maximumLineLength, allowTruncation = true)
      .map(_.utf8String.split("\\s*,\\s*"))

  // Select a specific column (including header) by name
  def selectCol(name: String): Flow[Row, String, Unit] = Flow[Row]
  ...
  lazy val formatRow = Flow[Row].map(row => ByteString(row.mkString("", ",", "\n")))
