...
class FlowGraphsSpec extends WordSpec with AkkaStreamsTest with Matchers with ScalaFutures {
  ...
  "quote.appendSma flow" should {
    ...
    "append SMA column" in {
      ...
      val future = inSource.via(csv.parseCsv().via(quote.appendSma(window)).via(csv.formatRow)).runWith(outSink)
      ...
      whenReady(future) { unit =>
        ...
        // Inspect row counts
        val countRows = csv.parseCsv().fold(0)((c, _) => c + 1).toMat(Sink.head)(Keep.right)
        ...
        val inFieldsFuture = inSource.via(csv.parseCsv()).runWith(Sink.head)
        val outFieldsFuture = outSource.via(csv.parseCsv()).runWith(Sink.head)
        ...
        val selectSma = csv.parseCsv().via(csv.selectCol(smaName)).drop(1).map(_.toDouble)
